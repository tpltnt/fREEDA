...
//Static members
const unsigned Mosnbsim3::n_par = 98;

//Element information
ItemInfo Mosnbsim3::einfo =
{
  "mosnbsim3",
...
Mosnbsim3::Mosnbsim3(const string& iname) : ADInterface(&einfo, pinfo, n_par, iname)
{
...
void Mosnbsim3::init() throw(string&)
{
...
void Mosnbsim3::eval(AD * x, AD * effort, AD * flow)
{
  ...
  if (diffVds > (beta0/EXP_THRESHOLD))
    T2 = (((alpha0 + alpha1 * leff)/leff) * diffVds * exp(-beta0 / diffVds));
  else
    T2 = ((((alpha0 + alpha1 * leff)/leff) * MIN_EXP) * diffVds);
