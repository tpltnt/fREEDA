...
// Static members
const unsigned Mospekv::n_par = 44;

// Element information
ItemInfo Mospekv::einfo =
{
  "mospekv",
...
Mospekv::Mospekv(const string& iname) : ADInterface(&einfo, pinfo, n_par, iname)
{
  // Set default parameter values
  paramvalue[0]  = &(type = PMOS);
  paramvalue[1]  = &(l = 1e-6);
  paramvalue[2]  = &(w = 1e-6);
  paramvalue[3]  = &(np = 1.0);
  paramvalue[4]  = &(ns = 1.0);
  ...
  paramvalue[17] = &(vfb = -2003);
  paramvalue[18] = &(uo = 0.0);
  paramvalue[19] = &(vmax = 0.0);
  paramvalue[20] = &(theta = -1);
...
void Mospekv::init() throw(string&)
{
  // Set Constants
...
void Mospekv::eval(AD * x, AD * effort, AD * flow)
{
  // x[0]: vds
  // x[1]: vgs
  // x[2]: vbs
  ...
  double deltavRSCE = 2 * qo / (cox * pow(1 + 0.5 * (xi + sqrt(xi*xi + cEps)),2));
  ...
  vpo1 = vgprime - phiT - gammaa * (sqrt(vgprime + gammaa*gammaa / 4) - gammaa
  / 2);
  if (vgprime > 0.0)
    vpo = vpo1;
  else
  ...
  / leff - 3 * weta * sqrt(vpo + phiT) / weff) / cox;
  gammaprime = 0.5 * (gammao + sqrt(gammao*gammao + 0.1 * vtT));
  vp1 = vgprime - phiT - gammaprime * (sqrt(vgprime+pow(gammaprime / 2,2)) -
  gammaprime / 2);
  if (vgprime > 0.0)
    vp = vp1;
  else
  ...
  deltav*deltav) + sqrt((vds-vdssprime)*(vds-vdssprime) + deltav*deltav)) / vtT)/ 2));
  ...
  nq = 1 + gammaa / (2 * sqrt(vp + phiT +1e-6));

  // Normalized intrinsic node charges
  xf = sqrt(0.25 + i_f);
  xr = sqrt(0.25 + ir);
  qd = -nq * (4 * (3 * xr*xr*xr + 6 * xr*xr * xf + 4 * xr * xf*xf + 2 * xf*xf*xf) / (15 *
  pow(xf + xr,2)) - 0.5);
  qs = -nq * (4 * (3 * xf*xf*xf + 6 * xf*xf * xr + 4 * xf * xr*xr + 2 * xr*xr*xr) / (15 *
  pow(xf + xr,2)) - 0.5);
  qi = qs + qd;
  qb1 = -gammaa * sqrt(vp + phiT + 1e-6) / vtT - (nq - 1) * qi / nq;
  if (vgprime > 0.0)
    qb = qb1;
  else
    qb = -vgprime / vtT;
