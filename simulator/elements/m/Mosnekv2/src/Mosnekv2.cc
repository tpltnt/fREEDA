...
const unsigned Mosnekv2::n_par = 40;

// Element information
ItemInfo Mosnekv2::einfo =
{
  "mosnekv2",
  "EPFL-EKV MOST Model",
...
Mosnekv2::Mosnekv2(const string& iname) : ADInterface(&einfo, pinfo, n_par, iname)
{
...
double vt(double temper)
{
	return 1.3807e-23*temper/1.602e-19; 			// (7) thermal voltage
	//return k*temper/q;
}

double eg(double temper)
{
	return 1.16-(7.02e-4*temper*temper)/(temper + 1108.0); 		// (8) [eV] energy gap
}

double ni(double temper, double trefer) 	// Intrinsic carrier consentration
{
	return 1.45e16*(temper/trefer)*exp(0.5*eg(trefer)/vt(trefer)-0.5*eg(temper)/(vt(temper))); // [m^-3] Intrinsic carrier concentration (9)
}

void Mosnekv2::init() throw(string&)
{
  // Set Constants
	...
	if(nsub>0)
	{
		phi=2*vt(tnom)*log((nsub*1e6)/ni(tnom, tref)); 	// (12)
	}
	...
	double phi(double phi, double temp, double tnom, double vt) // (20)
	{
		return phi*(temp/tnom)-3*vt*log(temp/tnom)-eg(tnom)*(temp/tnom)+eg(temp);
	}
...
void Mosnekv2::eval(AD * x, AD * effort, AD * flow)
{
  // Input Matrix
  // x[0]: vds
  // x[1]: vgs
  ...
  vprimes = 0.5*(vs+phi+sqrt((vs+phi)*(vs+phi)+(4*vt(temp))*(4*vt(temp)))); // (35)
  vprimed = 0.5*(vd+phi+sqrt((vd+phi)*(vd+phi)+(4*vt(temp))*(4*vt(temp)))); // (35b)

  gammadegree=gammaa-epsilonsi/cox*(leta/leff*(sqrt(vprimes)+sqrt(vprimed))-(3*weta)/weff*sqrt(vp0+phi)); //(36)

  gammaprime=0.5*(gammadegree+sqrt(gammadegree*gammadegree+0.1*vt(temp))); 	// (37)
  ...
  n=1+gammaa/(2*sqrt(vp+phi+4*vt(temp))); 			// (39)

  //=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  // Forward normalized current
	verytemp=(vp-vs)/vt(temp);
  ...
  vdss = vc*(sqrt(.25 + (vt(temp)/vc)*sqrt(isubf)) - .5); 	// (46)

  // Drain-to-source saturation voltage for reverse normalized current
  vdssprime = vc*(sqrt(.25 + (vt(temp)/vc)*(sqrt(isubf) - .75*log(isubf))) - .5) + vt(temp)*(log(vc/(2*vt(temp))) - .6); //(47)

  // Channel-length Modulation
  deltav = 4*vt(temp)*sqrt(lambda*(sqrt(isubf) - vdss/vt(temp)) + .015625); // (48)
  vds2 = (vd - vs)/2; 					// (49)
  ...
  // Uses smoothing function above called funct
  virprime = ((vp - vds2 - vs - sqrt(vdssprime*vdssprime + deltav*deltav) + sqrt((vds2 -vdssprime)*(vds2 - vdssprime) + deltav*deltav))/(vt(temp))); //(56a)
  ...
  ir=log(1+exp(vpdt/2))*log(1+exp(vpdt/2));
  {
	...
	//      {
		//      nu = 1/3;
	//      }  //(59) we took take care of this by creating separate NMOS and PMOS models
  ...
  /*
	...
	if (vgprime > 0)
	{
		qb = (-gammaa*sqrt(vp + phi + 0.000001)*(1/vt(temp)) - ((nq - 1)/nq)*qi;
	}
	else
	{
		qb = -vgprime*(1/vt(temp));
	} 						// (75)
  ...
  */
  ...
  /*
	...
	beta = beta0prime/(1 + cox/(e0*epsilonsi)*vt(temp)*sqrt((qb + nu*qi)*(qb + nu*qi))); // (62)
  */

  // Transconductance factor and mobility reduction due to vertical field cont'd
  //Easy way of calculating beta
  vpprime = .5*(vp + sqrt(vp*vp + 2*vt(temp)*vt(temp))); 		// (63)
  beta = beta0/(1 + theta*vpprime); 			// (64)

  // Specific current
  isp = 2*n*beta*vt(temp)*vt(temp); 				// (65)
  ...
  vib= vd-vs-ibn*2*vdss;  				//(67)
	...
	if (vib > 0)
    idb = ids*(iba/ibb)*vib*exp((-ibb*lc)/vib); // (68)
	else
		idb = 0;
