...
NLTrapezoidal::NLTrapezoidal(CircVector*& cX, CircVector*& cY,
CircVector*& cTime, double& h)
{
  ...
  a2 = a * a;
  da = 2. * a;
  xpn = DenseDoubleVector(cX->getCurrent().length());
  xsn = DenseDoubleVector(cX->getCurrent().length());
  ypn = DenseDoubleVector(cY->getCurrent().length());
}

NLTrapezoidal::NLTrapezoidal(CircVector*& cX, CircVector*& cY, CircVector*& cU,
CircVector*& cTime, double& h)
{
  this->cX = cX;
  this->cY = cY;
...
NLTrapezoidal::NLTrapezoidal(CircVector*& cX, CircVector*& cY, double& h)
{
  this->cX = cX;
  this->cY = cY;
  this->cTime=NULL;
...
{
  // first we need to find the where in the circular matrix the state variable
  // required is stored
  // double time_delay
  if(cTime==NULL)
	{
...
double NLTrapezoidal::derivY(const int& index, const double& currval)
{
  // Save Current value
  cY->getCurrent()[index] = currval;
  return (currval - cY->getPrevious(1)[index]) * a - ypn[index];
}
...
void NLTrapezoidal::predictX(int n_states, double *Integ_predX,
...
void NLTrapezoidal::predictU(int ls_size, double *Integ_predU,
CircVector*& cTimestep)
{
...
void NLTrapezoidal::store()
{
  ...
  int jend = ypn.length();
	...
	{
		ypn[i] = a * (cY->getCurrent()[i] - cY->getPrevious(1)[i]) - ypn[i];
	}
	return;
}
...
LTrapezoidal::LTrapezoidal(CircVector*& cU, TimeMNAM* mnam)
{
  ...
  upn = DenseDoubleVector(size);
  SerialCommunicator TrapeziodalComm;
  ProcessorMap Map(size, 0, TrapeziodalComm);
...
void LTrapezoidal::buildMd(DoubleSparseColMatrix& M, const double& h)
{
  a = 2. / h;
...
void LTrapezoidal::buildSf(DenseDoubleVector& s1, const double& ctime)
{
  assert(a);
  mnam->getSource(ctime, s2);
  
  DistributedDoubleVector u_n(Copy, M1p->RowMap(), &(cU->getPrevious(1)[0]));
  DistributedDoubleVector tmp1v(M1p->RowMap());
  
  for (int i = 0; i < size; i++)
    tmp1v[i] = upn[i] + u_n[i]*a;
  
  // multiply M1p with tmp1v and put the result in tmp1v
  M1p->Multiply(false, tmp1v, tmp1v);

  for (int i = 0; i < size; ++i)
    s1[i] = s2[i] + tmp1v[i];
}

void LTrapezoidal::buildSf(double* s1, const double& ctime)
{
  assert(a);
  mnam->getSource(ctime, s2);
...
void LTrapezoidal::multiply(DenseDoubleVector& u_n, double* s1)
{
  ...
  prev_ptr=tmp_ptr;
    ...
    if(tmp_ptr->row==prev_ptr->row)
      ...
      col_count=tmp_ptr->col;
			temp += tmp_ptr->val *( u_n[col_count] * a+upn[col_count]);
			prev_ptr=tmp_ptr;
...
void LTrapezoidal::changeStep(const double& h)
{
  a = 2. / h;
}
...
void LTrapezoidal::store()
{
  ...
  //cout << "\n\nRunning Trapezoidal store()....\n";  

  for (int i=0; i < size; i++)
  {
    //cout << "\ni = " << i << endl;
    //cout << "\nCurrent U: " << cU->getCurrent()[i] << endl;
    //cout << "\nPrevious U: " << cU->getPrevious(1)[i] << endl;
    //cout << "\nPrevious Deriv: " << upn[i] << endl;

    upn[i] = a * (cU->getCurrent()[i] - cU->getPrevious(1)[i]) - upn[i];

    //cout << "\nNew Deriv: " << upn[i] << endl;
