\element{E}{Voltage-Controlled Voltage Source}

\begin{figure}[h]
\centering
\ \pfig{e_spice.ps}
\caption{E --- voltage-controlled voltage source element.}
\end{figure}

\form{
     {\tt E}name $N_{+}$ $N_{-}$ $N_{C+}$ $N_{C-}$ Gain\\
     {\tt E}name $N_{+}$ $N_{-}$ {\tt POLY(} D {\tt )} $N_{C+}$ $N_{C-}$
...
\pspiceninetytwoform{
     ...
     ... {\tt (}$N_{CD+}$ $N_{CD-}{\tt )}$
        PolynomialCoefficients
     {\tt E}name $N_{+}$ $N_{-}$ {\tt VALUE= \{} Expression {\tt \}}\\
...
\item Several form of the voltage-controlled voltage source element are supported
      in addition to the \underline{Linear Gain} form which is the default.
