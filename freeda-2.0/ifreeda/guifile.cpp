...
//#include "components/spicefile.h" QUCS-EDIT REMOVE SPICEFILE
#include "guidoc.h"
#include "guimain.h"
...
bool QucsFile::loadIntoNothing(Q3TextStream *stream)
{
  QString Line, cstr;
  while(!stream->atEnd()) {
...
bool QucsFile::pasteFromClipboard(Q3TextStream *stream, Q3PtrList<Element_Qucs> *pe)
{
  QString Line;
...
// -------------------------------------------------------------
bool QucsFile::loadProperties(Q3TextStream *stream)
{
  bool ok = true;
...
// -------------------------------------------------------------
bool QucsFile::loadComponents(Q3TextStream *stream, Q3PtrList<Component> *List)
{
  QString Line, cstr;
  Component *c;
  while(!stream->atEnd()) {
...
// -------------------------------------------------------------
bool QucsFile::loadWires(Q3TextStream *stream, Q3PtrList<Element_Qucs> *List)
{
  Wire *w;
  QString Line;
  while(!stream->atEnd()) {
...
// -------------------------------------------------------------
bool QucsFile::loadDiagrams(Q3TextStream *stream, Q3PtrList<Diagram> *List)
{
  Diagram *d;
  QString Line, cstr;
  while(!stream->atEnd()) {
...
// -------------------------------------------------------------
bool QucsFile::loadPaintings(Q3TextStream *stream, Q3PtrList<Painting> *List)
{
  Painting *p=0;
  QString Line, cstr;
  while(!stream->atEnd()) {
...
bool QucsFile::giveNodeNames(Q3TextStream *stream, int& countInit,
                             QStringList& Collect, Q3TextEdit *ErrText)
{
  Node *p1, *p2;
  Wire *pw;
  Element_Qucs *pe;
  ...
  for(Component *pc = Comps->first(); pc != 0; pc = Comps->next())
  {
    ...
    {
      if(pc->Model == "GND") 
        pc->Ports.getFirst()->Connection->Name = "0";
      else if(pc->Model == "Sub") 
      {
         ...
         {  
	       ...
	       d->DocName = s;
  	     bool r = d->File.createSubNetlist(stream, countInit, Collect, ErrText);
	       delete d;
	       if(!r) return false;
      }
      else if(pc->Model == "SPICE") 
      {
	     ...
	     if(s.isEmpty()) 
       {
         ErrText->insert(QObject::tr("ERROR: No file name in SPICE component \"%1\".").arg(pc->Name));
	       return false;
	     }
	     if(StringList.findIndex(s) >= 0)
  ...
  for(p1 = Nodes->first(); p1!=0; p1 = Nodes->next()) { // work on all nodes
    ...
    p1->Name = "" + QString::number(z++);   // create node name
    if(p1->Label)
      if(!p1->Label->initValue.isEmpty())
	Collect.append("NodeSet:NS" + QString::number(countInit++) + " " + p1->Name +
			" U=\"" + p1->Label->initValue + "\"");
...
bool QucsFile::createSubNetlist(Q3TextStream *stream, int& countInit,
                                QStringList& Collect, Q3TextEdit *Error)
{
  int i, z,k=0;
  QString s;
  ...
  for(pc = Comps->first(); pc != 0; pc = Comps->next())
    if(pc->Model == "Port") {
      ...
      for(z=sl.size(); z<i; z++)
	      sl.append(" ");
      // it = sl.at(i-1);
      // Added by Shivam to iterate. Now At function doesnot return pointer
      // where as it returns value at index.  So for iteration for loop has been
      // written
      for(it = sl.begin(); k != (i-1) ; ++k) ++it;
...
bool QucsFile::prepareNetlist(Q3TextStream& stream, QStringList& Collect,
                              Q3TextEdit *Error)
{
  // first line is documentation
  stream << "# ifREEDA " << PACKAGE_VERSION << "  " << Doc->DocName << "\n";
...
// .................................................
// write all components with node names into the netlist file
void QucsFile::createNetlist(Q3TextStream& stream)
{
  QString s;
  QStringList DotOutStatements;
  QStringList ModelStatements;
  QStringList AnalysisStatements;
  QStringList ElementStatements;
  QStringList TopStatements;
  QStringList BottomStatements;
  
  bool subcircuit = false;
  int  dotOutInString    = 0;
  int  dotModelInString  = 0;
  int  dotTopInString    = 0; 
  int  dotBottomInString = 0;  
  int  analysisInString  = 0;
    
  for(Component *pc = Comps->first(); pc != 0; pc = Comps->next()) 
  {
    s = pc->NetList();
    // Changed by Shivam Contains of Qt3.3 is same as count of Qt4.5
    dotOutInString    = s.count(".out");
    dotModelInString  = s.count(".model");
    dotTopInString    = s.count(".top");
    dotBottomInString = s.count(".bottom");
    
    if (s.contains(".ac") || s.contains(".dc") || s.contains(".svhb") ||
        s.contains(".svtr") || s.contains(".tran") || s.contains(".tran2") ||
	s.contains(".tran3") || s.contains(".wavtran"))
    {
       analysisInString = 1;
    }
    else
    ...
    {  
      if (dotOutInString != 0)
      {//Save the .out statement to print at end of netlist.
	 DotOutStatements << s;
      }
     /*else if (analysisString != 0)
      {
	 analysis = QDeepCopy<QString> s;
      }*/
      else if (dotModelInString != 0)
      {
         ModelStatements << s;
      }
      else if (analysisInString != 0)
      {
         AnalysisStatements << s;
      }
      else if (dotTopInString != 0)
      {
         TopStatements << s.remove(0,4); //remove the .top so it will not be place in the netlist
      }
      else if (dotBottomInString != 0)
      {
         BottomStatements << s.remove(0,7);//remove the .bottom so it will not be place in the netlist
      }      
      else
      { 
	ElementStatements << s;
  ...
  for ( QStringList::Iterator it = TopStatements.begin(); it != TopStatements.end(); ++it ) 
  {//Now add the .model statements to the top of the file.
        stream << *it << "\n";
  }

  stream << "\n" << "\n";  
  for ( QStringList::Iterator it = ModelStatements.begin(); it != ModelStatements.end(); ++it ) 
  {//Now add the .model statements to the top of the file.
        stream << *it << "\n";
  }

  stream << "\n" << "\n";
  for ( QStringList::Iterator it = ElementStatements.begin(); it != ElementStatements.end(); ++it ) 
  {//Now add the element statements to the file.
        stream << *it << "\n";
  }
  
  stream << "\n" << "\n";
  for ( QStringList::Iterator it = AnalysisStatements.begin(); it != AnalysisStatements.end(); ++it ) 
  {//Now add the analysis statements to the file.
        stream << *it << "\n";
  }
  
  stream << "\n" << "\n";
  for ( QStringList::Iterator it = DotOutStatements.begin(); it != DotOutStatements.end(); ++it ) 
  {//Now add the .out statements to the end of the file.
        stream << *it << "\n";
  }
  
  stream << "\n" << "\n";

  //Test if this is a subcircuit. If it is .ends should be in the BottomStatements
  int index = -1;
  index = BottomStatements.findIndex(".ends");
  if (index != -1)
     subcircuit = true;
     
  for ( QStringList::Iterator it = BottomStatements.begin(); it != BottomStatements.end(); ++it ) 
  {//Now add the .out statements to the end of the file.
	stream << *it << "\n";
  }
