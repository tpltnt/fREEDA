...
QucsDoc::QucsDoc(QucsApp *App_, const QString& _Name) : File(this)
{
  ...
  DocName = _Name;
  //  Added by Shivam.  The Usage of QTab* is removed.
  App = App_;
  if(App)
       Bar = App->WorkView;
  else
  ...
  if(_Name.isEmpty())  //  Tab = new QTab(QObject::tr("untitled"));  Commented by Shivam
       {
          if(Bar){  tab_new = Bar->addTab(QObject::tr("untitled"));
                   Bar->repaint(); 
                 }
       }       
  else {
    ...
    if(Bar) { tab_new = Bar->addTab(Info.fileName());
              Bar->repaint();
            } 
 ...
 /* App = App_;
  ...
  } else Bar = NULL;  */   // Commented by Shivam and included in above code.
...
void QucsDoc::setName(const QString& _Name)
{
 ...
 //  Added by Shivam
     Bar->setTabText(tab_new,Info.fileName());
...
void QucsDoc::setChanged(bool c, bool fillStack, char Op)
{
  if((!DocChanged) && c) {
    //Tab->setIconSet(QPixmap(smallsave_xpm));
     Bar->setTabIcon(tab_new,QPixmap(smallsave_xpm));
    if(Bar) {
      //Bar->layoutTabs();
      //Bar->tabLayoutChange();
      Bar->repaint();
    ...
    //Tab->setIconSet(QIcon(0));   // no icon in TabBar
       Bar->setTabIcon(tab_new,QIcon());  // Construct a null icon
    if(Bar) {
      //Bar->layoutTabs();    // Done automatically now
      Bar->repaint();
    }
  }
  DocChanged = c;
...
Node* QucsDoc::insertNode(int x, int y, Element_Qucs *e)
{
  Node *pn;
...
void QucsDoc::insertRawComponent(Component *c, bool num)
{
  ...
  if(c->Model == "GND") {
    ...
    Element_Qucs *pe = getWireLabel(c->Ports.getFirst()->Connection);
    if(pe) {
      if(pe->Type == isWire) {
    ...
    else if(ptr2->y1 == w->y1) 
    {
      ...
      if(ptr2->isHorizontal() == w->isHorizontal()) // (ptr2-wire is horizontal)
      {
        ...
        { 
          delete w; 
          return 0; 
        }  // new wire lies within an existing wire
        else 
        {
    	    // one part of the wire lies within an existing wire
      	  // the other part not
...
void QucsDoc::selectWireLine(Element_Qucs *pe, Node *pn, bool ctrl)
{
  Node *pn_1st = pn;
...
Element_Qucs* QucsDoc::selectElement(int x, int y, bool flag, int *index)
{
  int n;
  Element_Qucs *pe_1st=0, *pe_sel=0;
...
void QucsDoc::deselectElements(Element_Qucs *e)
{
  // test all components
  for(Component *pc = Comps->first(); pc != 0; pc = Comps->next())
...
void QucsDoc::NewMovingWires(Q3PtrList<Element_Qucs> *p, Node *pn)
{
  if(pn->Connections.count() < 1) return; // return, if connected node moves
  Element_Qucs *pe = pn->Connections.getFirst();
  if(pe == (Element_Qucs*)1) return; // return, if it was already treated this way
  pn->Connections.prepend((Element_Qucs*)1);  // to avoid doubling

  if(pn->Connections.count() == 2)    // 2, because of prepend (Element_Qucs*)1
    if(pe->Type == isWire) {    // is it connected to exactly one wire ?
      ...
      if(pn2->Connections.count() == 2) { // two existing wires connected ?
        Element_Qucs *pe2 = pn2->Connections.getFirst();
        if(pe2 == pe) pe2 = pn2->Connections.getLast();
        if(pe2 != (Element_Qucs*)1)
      ...
      if(pw->Port1->Connections.getFirst() !=  (Element_Qucs*)1)
        pw->Port1->Connections.prepend((Element_Qucs*)1);  // remember this action
      if(pw->Port2->Connections.getFirst() !=  (Element_Qucs*)1)
        pw->Port2->Connections.prepend((Element_Qucs*)1);  // remember this action
	...
	pw->Port2 = (Node*)mask;
      ...
      if(pw2->Port1->Connections.getFirst() !=  (Element_Qucs*)1)
        pw2->Port1->Connections.prepend((Element_Qucs*)1); // remember this action
      if(pw2->Port2->Connections.getFirst() !=  (Element_Qucs*)1)
        pw2->Port2->Connections.prepend((Element_Qucs*)1); // remember this action
...
void QucsDoc::copySelectedElements(Q3PtrList<Element_Qucs> *p)
{
  Port      *pp;
  Component *pc;
  Wire      *pw;
  ...
  for(pc = Comps->first(); pc != 0; )
    if(pc->isSelected) {
      ...
      for(pp = pc->Ports.first(); pp!=0; pp = pc->Ports.next())
        pp->Connection->Connections.removeRef((Element_Qucs*)pc);
  ...
  for(pn = Nodes->first(); pn!=0; ) {
    if(pn->Connections.getFirst() == (Element_Qucs*)1) {
...
int QucsDoc::placeNodeLabel(WireLabel *pl)
{
  ...
  Element_Qucs *pe = getWireLabel(pn);
  if(pe) {    // name found ?
...
// labeled element.
Element_Qucs* QucsDoc::getWireLabel(Node *pn_)
{
  Wire *pw;
  Node *pn;
  Element_Qucs *pe;
...
void QucsDoc::setCompPorts(Component *pc)
{
  ...
  for(Port *pp = pc->Ports.first(); pp!=0; pp = pc->Ports.next()) {
    pp->Connection->Connections.removeRef((Element_Qucs*)pc);// delete connections
...
bool QucsDoc::rotateElements()
{
  ...
  WireLabel *pl;
  // re-insert elements
  for(Element_Qucs *pe = ElementCache.first(); pe != 0; pe = ElementCache.next())
    switch(pe->Type) {
      case isComponent:
           pc = (Component*)pe;
...
bool QucsDoc::paste(Q3TextStream *stream, Q3PtrList<Element_Qucs> *pe)
{
...
bool QucsDoc::load()
{
  ...
  // Changed by Shivam
  (*(UndoStack.current()))[1] = 'i';  // state of being unchanged
...
int QucsDoc::save()
{
  ...
  if(result >= 0) {
    ...
    for(p = UndoStack.first(); p != 0; p = UndoStack.next()) 
      (*p)[1] = QChar(' ');   //p->at(1) = ' ';  // state of being changed
      (*ps)[1] = QChar('i');       //ps->at(1) = 'i';   // state of being unchanged
      qDebug(" String is : %s \n",(*ps).toAscii().data());
    ...
    for(p = UndoSymbol.first(); p != 0; p = UndoSymbol.next())
      // Changed by Shivam
    (*p)[1] = QChar(' ');  // state of being changed
    (*ps)[1] = QChar('i');   // state of being unchanged
