...
X::X(const string& iname) : Element(&einfo, NULL, n_par, iname)
{
	circuit_def = NULL;
...
void X::attachDefinition(Circuit *circuit)
{
  assert(!circuit_def);
  circuit_def = circuit;
  circuit_def->addUser();
...
void X::expandToCircuit(Circuit* target_c)
{
  assert(target_c);
