...
// Static members
const unsigned Zbutter::n_par = 11 ;

// Element information
ItemInfo Zbutter::einfo =
{
  "zbutter",
  "Z-Domain Butterworth Lowpass or Bandpass Filter",
  "Frank P. Hart",
  DEFAULT_ADDRESS"elements/zbutter.h.html"
};

// Parameter information
ParmInfo Zbutter::pinfo[] =
{
  ...
  {"pbw", "Passband bandwidth (Hz or %)",TR_DOUBLE, false},
...
Zbutter::Zbutter(const string& iname) : Element(&einfo, pinfo, n_par, iname)
{
...
Zbutter::~Zbutter() // 'delete' in reverse order of 'new'
{
  if (lowpass)
  {
    delete [] y_int;
...
void Zbutter::getLocalRefIdx(UnsignedVector& local_ref_vec,
...
void Zbutter::init() throw(string&)
{
...
void Zbutter::svTran(TimeDomainSV* tdsv)
{
  if(tdsv->DC()) {
  ...
  else
  {
    ...
    if (first_iteration)
    { // new first iter, so *can't* set x_last to
...
void Zbutter::deriv_svTran(TimeDomainSV* tdsv)
{
  tdsv->getJu()(0,0) = one;
  tdsv->getJu()(0,1) = zero;
  //tdsv->getJu()(1,0) = zero;
...
void Zbutter::eval(double* x, double* vp, double* ip, const TimeDomainSV* tdsv)
{
...
void Zbutter::filter_init()
{
  ...
  if (Ngiven)
  { // don't really need to set sbadb, could let sbw vary
    // sbadb=-40.0;
    N = pord;
  }
  ...
  if (Ngiven)
  {
    ...
    sbw = pbw*pow(lambda/epsilon,1.0/Nd);
  ...
  if (Ngiven==false)
  {
    N = static_cast<int>(ceil(log10(epsilon/lambda)/log10(wp/ws)));
  }
  wc = wp/(pow(epsilon,(1.0/N)));
  ...
  {
    plp_den1[k] = 1.0;
    plp_den2[k] = wc*2.0*sin((2.0*k+1)*pi/(2.0*N));
    plp_den3[k] = wc*wc;
  }
  if (Nodd)
  {
  ...
  if (lowpass)
  {
    ...
    if (Nodd)
    {
      z_den1[kmax-1] = 1.0 + wc;
      z_den2[kmax-1] = wc - 1.0;
      z_den3[kmax-1] = 0.0;
    }
  ...
  { // bandpass
    ...
    if (Nodd)
    {
      z_den1[kmax-1] = b + c + b*wo2;
      z_den2[kmax-1] = 2.0*b*(wo2-1.0);
      z_den3[kmax-1] = b - c + b*wo2;
      z_den4[kmax-1] = 0.0;
      z_den5[kmax-1] = 0.0;
    }
  ...
  // Debugging...
  if (!rep)
  { // put debugging info in std output only...
    cout << endl;
    cout << "****************************************************"
      << endl;
    cout << "Info for Zbutter instance: " << name_ch;
    cout << endl;
    cout << endl;
  ...
  { // if (report)
    ...
    sepLine();
    newLine();
    sprintf(msg,"Info for Zbutter instance: %s",
        name_ch );
    report(MESSAGE, msg);
    newLine();
