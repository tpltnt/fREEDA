#include "Mosnbsim3SOI5T1.h"

//Static members
const unsigned Mosnbsim3SOI5T1::n_par = 202;

//Element information
ItemInfo Mosnbsim3SOI5T1::einfo =
{
  "mosnbsim3soi5t1",
...
//Parameters
ParmInfo Mosnbsim3SOI5T1::pinfo[] =
{
  ...
  {"cjswg", "Source/Drain (gate side) sidewall junction capacitance per unit width (normalized to 100nm tsi) (F/m^2)", TR_DOUBLE, false},
...
//   {"ndiode", "ndiode", TR_DOUBLE, false},
//   {"selfheat", "self heat - shmod", TR_DOUBLE, false},
};

Mosnbsim3SOI5T1::Mosnbsim3SOI5T1(const string& iname) : ADInterface(&einfo, pinfo, n_par, iname)
{
...
void Mosnbsim3SOI5T1::init() throw(string&)
{
...
void Mosnbsim3SOI5T1::eval(AD * x, AD * effort, AD * flow)
{
  ...
  AD qinv, qsrc, qgate, qbody, qsub, qdrn, Qe1;
  AD T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14, TMP, TMP3, TMP4, TMP11, TMP12, TMP13;
  double t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, tmp, tmp1, tmp2, tmp3, tmp4, vbsc, V0, T00, TT0;
  AD Esat, EsatL;
  ...
  double pdbcp = 0;
  double psbcp = 0;
  double wdiod = weff / nseg + pdbcp;
  double wdios = weff / nseg + psbcp;
  ...
  double TempRatioMinus1 = (TempRatio - 1.0);
  T00 = TempRatioMinus1;
  double uatemp = ua;
  double ubtemp = ub;
  double uctemp = uc;
  double rds0denom = pow(weff * 1e6, wr);
  double rth = rth0 / (weff + wth0) * nseg;
  double cth = cth0 * (weff + wth0) / nseg;
  double frbody = 1.0;
  double rbody = frbody * rbody * rhalo / (2 * rbody + rhalo * leff) * weff / nseg;
  double oxideRatio = pow(toxref/toxqm, ntox) / toxqm / toxqm;
  double GatesidewallJctPotential = 0.9;
  double bodyJctGateSideGradingCoeff = 0.5;
  double unitLengthGateSidewallJctCap = 8.0e-10;
  double tt = 1e-12;
  double sourceArea = 0.0;
  double drainArea =0.0;
  double Cbox = 3.453133e-11 / tbox; //---Check
  double csbox = Cbox * sourceArea;
  double cdbox = Cbox * drainArea;
  double csmin = Cbox * sourceArea;
  double cdmin = Cbox * drainArea;
  AD cjsbs, dcjsbs_dT, cjdbs,  dcjdbs_dT,  DioMax,arg, PhiBSWG,MJSWG,dT3_dVb,  qjs, qjd;
  AD SDphi, qse, qde;
  AD qgd, qgs, qge;
  ua = ua + ua1 * TempRatioMinus1 ;
  ub = ub + ub1 * TempRatioMinus1;
  uc = uc + uc1 * TempRatioMinus1;
  ...
  /* v2.2.2 bug fix */
  double ahli0 = ahli * TT0;
  double jbjt = isbjt * TT0;
  double jdif = isdif * t1;
  double jrec = isrec * t2;

  t7 = xtun * (TempRatio -1);
  DEXP1(t7, TT0);
  double jtun = istun * TT0;
  double vfbb;
  double vsdfb;

  if (nsub > 0)
    vfbb = -type * vtm * log(nch/ nsub);
  else
    vfbb = -type * vtm * log(-nch* nsub/ni/ni);
  ...
  double phi = 2.0 * vtm * log(npeak / ni);
  double sqrtPhi = sqrt(phi);
  double phis3 = sqrtPhi * phi;
  double Xdep0 = sqrt(2.0 * EPSSI / (Charge_q * npeak * 1.0e6)) * sqrtPhi; //replaced nch with npeak
  double sqrtXdep0 = sqrt(Xdep0);
  double litl = sqrt(3.0 * xj * tox);
  double vbi = vtm * log(1.0e20 * nch / (ni * ni));
  double cdep0 = sqrt(Charge_q * EPSSI * npeak * 1.0e6 / 2.0 / phi);
  ...
  double ToxRatio, ToxRatioEdge, Aechvb, Bechvb, AechvbEdge, BechvbEdge;
  double poxedge = 1.0;
  ToxRatio = exp(ntox * log(toxref /toxqm)) /toxqm /toxqm;
  ToxRatioEdge = exp(ntox * log(toxref / (toxqm * poxedge))) / toxqm / toxqm / poxedge / poxedge;
  Aechvb = (type == 1.0) ? 4.97232e-7 : 3.42537e-7;
  Bechvb = (type == 1.0) ? 7.45669e11 : 1.16645e12;
  AechvbEdge = Aechvb * weff/nseg * Dlcig * ToxRatioEdge; /* v3.1 bug fix */
  BechvbEdge = -Bechvb * toxqm * poxedge;
  ...
  //until pg 15 of b3soitemp.c - adding it below:
  ...
  if ( ((nsub > 0) && (type > 0)) ||((nsub < 0) && (type < 0)) )
  {
    ...
    st2 = t2 / asd;
    st3 = t2 /( 1.0 - asd);
    st4 = t0 * t1 * (1.0 + asd) / 3.0 - csmin * vsdfb;
    t1 = cdbox - cdmin;
    t2 = t1 / t0 / t0;
    dt2 = t2 / asd;
    dt3 = t2 /( 1.0 - asd);
    dt4 = t0 * t1 * (1.0 + asd) / 3.0 - cdmin * vsdfb;
  }
  else
  {
    t0 = vsdfb - vsdth;
    ...
    st2 = t2 / asd;
    st3 = t2 /( 1.0 - asd);
    st4 = t0 * t1 * (1.0 + asd) / 3.0 - csbox * vsdth;
    t1 = cdmin - cdbox;
    t2 = t1 / t0 / t0;
    dt2 = t2 / asd;
    dt3 = t2 /( 1.0 - asd);
    dt4 = t0 * t1 * (1.0 + asd) / 3.0 - cdbox * vsdth;
  }

  double arfabjt, lratio, lratiodif, vearly, ldeb;
  ...
  double qsi = Charge_q * npeak * (1.0 + nlx / leff) * 1e6 * tsi;
  ldeb = sqrt(EPSSI * Vtm0 / (Charge_q * nch * 1.0e6)) / 3.0;
  double csi = 1.03594e-10 / tsi;
  ...
  t0 = vfb + phi;
  if((ngate > 1.e18) && (ngate < 1.e25) && (x[1] > t0))
  {
  ...
  DeltVthtemp = k1eff * (t0 - 1.0) * sqrtPhi + T1 * T00;
  ...
  k2 = 0.0;
  AD VthFD = type * vth0 + k1eff * (sqrtPhis - sqrtPhi) - k2 * Vbs0mos - Delt_vth - DeltVthw + (k3
  + k3b * Vbs0mos) * tmp2 + DeltVthtemp - DIBL_Sft; //--Not changed '1.0' to "type" parameter
  ...
  //0.9*Rds hack for thermal simulations to match
  T0 = 1.0 + gche *Rds;
  T9 = Vdseff / T0;
  AD Idl = gche * T9;
  T9 = diffVds / Va;
  ...
  double WsTsi = wdios * tsi;
  double WdTsi = wdiod * tsi;
  double NVtm1 = Vtm * ndiode;
  AD E2ndFactor , ic, Ic;
  T0 = x[2] / NVtm1;
  ...
  double NVtmf = 0.026 * nrecf0 * (1 + ntrecf * (TempRatio - 1));
  double NVtmr = 0.026 * nrecr0 * (1 + ntrecr * (TempRatio - 1));
  ...
  else
  {
    ...
    double Iendif = WTsi * jbjt * lratiodif;
  ...
  double alphaGB1 = 0.35;
  double betaGB1 = 0.03;
  double alphaGB2 = 0.43;
  double betaGB2 = 0.05;
  ...
  {
    double dtoxcv = 0.0;
    double Cox = 3.453133e-11 / (tox - dtoxcv);
    double agbcp = 0.0;
    double CoxWL  = cox * (weffCV / nseg *leffCV + agbcp);
    CoxWL = CoxWL * tox / (tox- dtoxcv);
    double CoxWLb = CoxWLb * tox / (tox - dtoxcv);
    double  Tox = 1.0e8 * (tox - dtoxcv);
  ...
  AD qgdo = (dcgdo + T3) * (x[1] - x[2]) - T3 * (T2+ 0.5 * ckappa * (T4 - 1.0));
  T0 = x[1] + DELTA_1;
  T1 = sqrt(T0 * T0 + 4.0 * DELTA_1);
  T2 = 0.5 * (T0 - T1);
  ...
  AD qgso = (Dcgso + T3) * x[1] - T3 * (T2 + 0.5 * ckappa * (T4 - 1.0));
