...
// Static members
const unsigned Mosn1::n_par = 34;

// Element information
ItemInfo Mosn1::einfo =
{
  "mosn1",
...
Mosn1::Mosn1(const string& iname) : ADInterface(&einfo, pinfo, n_par, iname)
{
...
void Mosn1::eval(AD * x, AD * effort, AD * flow)
{
  // x[0]: vds
  // x[1]: vgs
  // x[2]: vbs
  ...
  // No test for inverse connection of source and
	// drain nodes from node convention
  ...
  term1 = alphax * x[0] * x[0]
	/ (4.0 * type * (x[1] - vth - alphax * x[0] / 2.0));
  ...
  // Subthreshold region
    ...
    qdrain = 0;
    qsource = 0;
    qbulk = -coxwl*gammasqr/2.0*(-1.0 + sqrt((1.0 + 4.0*type*(x[1]
		- vfb - x[2]))/gammasqr));
    qgate = -(qdrain + qsource + qbulk);
  }
  // Linear region
    ...
    qbulk = coxwl*( (type * (vfb  - vth + (1.0-alphax)*x[0]/2) + tphi)
		- (1.0-alphax)*term1/3.0);
    qgate = -(qdrain + qsource + qbulk);
  }
  // Saturation region
  else
    ...
    qbulk = coxwl * (type * (vfb - vth + (1.0-alphax)
		*(x[1] - vth)/(3.0*alphax)) + tphi) ;
    qgate = -(qdrain + qsource + qbulk);
	}
