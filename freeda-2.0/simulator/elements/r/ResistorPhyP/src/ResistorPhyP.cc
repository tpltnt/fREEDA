#include "ResistorPhyP.h"

// Static members
const unsigned ResistorPhyP::n_par = 23;

// Element information
ItemInfo ResistorPhyP::einfo =
{
  "resistorphyp",
...
ResistorPhyP::ResistorPhyP(const string& iname) : ADInterface(&einfo, pinfo, n_par, iname)
{
...
void ResistorPhyP::init() throw(string&)
{
...
void ResistorPhyP::eval(AD * x, AD * effort, AD * flow)
{
  ...
  effort[0] = -1.0 * (vd1 + id1*rs);
  effort[1] = -1.0 * (vd2 + id2*rs);
