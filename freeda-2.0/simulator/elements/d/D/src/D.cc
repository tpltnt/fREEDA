...
ParmInfo D::pinfo[] =
{
	...
	//  {"ind", "Internal inductor (only for svtr) (H)", TR_DOUBLE, false}
};


D::D(const string& iname) : ADInterface(&einfo, pinfo, n_par, iname)
{
	...
	// Set initial state for charge
	// qold = qnew = zero;
}


void D::init() throw(string&)
{
...
void D::eval(AD * x, AD * effort, AD * flow)
{
	// x[0]: state variable
