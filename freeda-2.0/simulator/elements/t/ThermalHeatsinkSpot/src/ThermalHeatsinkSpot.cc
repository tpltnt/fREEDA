...
const unsigned ThermalHeatsinkSpot::n_par = 17;
// Stefan-Boltzmann constant W.m-2.K-4.
const double ThermalHeatsinkSpot::sigma = 5.67051e-8;

// Element information
ItemInfo ThermalHeatsinkSpot::einfo =
{
  "thermalheatsinkspot",
...
ParmInfo ThermalHeatsinkSpot::pinfo[] =
{
...
ThermalHeatsinkSpot::ThermalHeatsinkSpot(const string& iname) : Element(&einfo, pinfo, n_par, iname)
{
  // Setup parameters
...
ThermalHeatsinkSpot::~ThermalHeatsinkSpot()
{
  if (Pstore)
...
void ThermalHeatsinkSpot::init() throw(string&)
{
  FILE *fp;
  double input;
  ...
  if (time_d) {
    ...
    Rth = newmatrix(0,Ntimesteps,1,Nsquared); /* Dynamically allocate
		storage for thermal
		impedance vector
...
unsigned ThermalHeatsinkSpot::getExtraRC(const unsigned& eqn_number,
const MNAMType& type)
{
...
void ThermalHeatsinkSpot::getExtraRC(unsigned& first_eqn, unsigned& n_rows) const
{
  assert(my_row[0]);
  first_eqn = my_row[0];
  n_rows = Nsquared;
...
void ThermalHeatsinkSpot::fillMNAM(FreqMNAM* mnam)
{
...
void ThermalHeatsinkSpot::svTran(TimeDomainSV* tdsv)
{
  if(tdsv->DC()) {
    for (int i=0; i < Nsquared; i++) {
    ...
    for (int  i = 1; i <= Nsquared; i++){
      ...
      if (tdsv->firstTime()) {
				...
				for (int m = n; m > 1; m--){
					storedDeltaT[i-1] += (Rth[m][i] - Rth[m-1][i])*Pstoretotal[n-m+1];
				}
      }
      double deltaTi = storedDeltaT[i-1]
			+ (Rth[1][i] - Rth[0][i]) * Pstoretotal[n];
...
void ThermalHeatsinkSpot::deriv_svTran(TimeDomainSV* tdsv)
{
  tdsv->cleanJac();
...
impedance. */
double ThermalHeatsinkSpot::LaplaceDomain(int i, double s)
{
...
double ThermalHeatsinkSpot::TimeIndependent(int i)
{
  int m,n,delta_m0,delta_n0,mMAX,nMAX;
  double result, P, lambda_m,mu_n,gamma_mn, H, T0, k, Imni,I00i,Imnj,I00j;
...
double_complex ThermalHeatsinkSpot::LaplaceDomain(int i, double_complex s)
{
  int m,n,delta_m0,delta_n0,mMAX,nMAX;
...
void ThermalHeatsinkSpot::TimeDomain(double t, double *Rthtemp)
{
  int v,i;
  ...
  for (i = 1; i <= Nsquared; i++){
    increment = result = 0;
    for (v=1;v<=Np;v++){
      increment = CalculateWeight(v) * LaplaceDomain(i,v*LN2/t);
...
Laplace inversion. */
double ThermalHeatsinkSpot::CalculateWeight(int v)
{
...
double ThermalHeatsinkSpot::fact(int x)
{
  int i;
  double cumul = 1;
...
void ThermalHeatsinkSpot::nrerror(const char *error_text)
{
...
void ThermalHeatsinkSpot::free_matrix(double** m, int nrl, int nrh, int ncl, int nch)
{
...
double** ThermalHeatsinkSpot::newmatrix(int nrl, int nrh, int ncl, int nch)
{
...
double* ThermalHeatsinkSpot::newvector(int nl,int nh)
{
  double *v;
...
void ThermalHeatsinkSpot::free_vector(double *v,int nl,int nh)
{
