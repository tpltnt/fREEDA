...
const unsigned Cmos2Nand :: n_par = 71;

// Element Information
ItemInfo Cmos2Nand::einfo =
{
  "cmos2nand",
...
// Constructor
  Cmos2Nand::Cmos2Nand(const string& iname)
:ADInterface(&einfo, pinfo, n_par, iname)
{
  ...
  paramvalue[8] = &(cox_n = 8.58e-3);
  paramvalue[9] = &(cox_p = 8.80e-3);
  paramvalue[10] = &(xj_n =  30.0e-9);
  paramvalue[11] = &(xj_p = 50.0e-9);
  paramvalue[12] = &(dw_n = -45.3e-9);
  paramvalue[13]= &(dw_p =  -7.2e-9);
  paramvalue[14]= &(dl_n =  -16.7e-9);
  paramvalue[15]= &(dl_p = -24.2e-9);
  paramvalue[16]= &(vto_n = 400.0e-3);
  paramvalue[17]= &(vto_p = -400.0e-3);
  paramvalue[18]= &(gamma_n = 300.0e-3);
  paramvalue[19]= &(gamma_p =610.0e-3);
  paramvalue[20]= &(phi_n = 450.0e-3);
  paramvalue[21]= &(phi_p = 452.0e-3);
  paramvalue[22]= &(kp_n =  390.0e-6);
  paramvalue[23]= &(kp_p = 82.0e-6);
  paramvalue[24] = &(eo_n = 438.0e+6);
  paramvalue[25] = &(eo_p = 2.10e+6);
  paramvalue[26] = &(ucrit_n = 5.0e6);
  paramvalue[27] = &(ucrit_p =5.5e6);
  paramvalue[28] = &(tox_n = 0.0);
  paramvalue[29] = &(tox_p = 0.54);
  ...
  paramvalue[40] = &(weta_n = 1.0);
  paramvalue[41] = &(weta_p = 1.0);
  paramvalue[42] = &(leta_n = 1.3);
  paramvalue[43] = &(leta_p = 1.0);
  ...
  paramvalue[54] = &(tcv_n =  600.0e-6);
  paramvalue[55] = &(tcv_p = -1.250e-3);
  paramvalue[56] = &(bex_n = -1.6);
  paramvalue[57] = &(bex_p = -850.0e-3);
  paramvalue[58] = &(ucex_n = 1.2);
  paramvalue[59] = &(ucex_p = 1.7);
...
void Cmos2Nand::init() throw(string&)
{
  // Set Constants
  scale=1.0;
...
// Evaluate Function
void Cmos2Nand::eval(AD * x, AD * effort, AD * flow)
{
  ...
  AD QS_n1, QG_n1,QS_p1,QG_p1,QS_n2,QG_n2,QS_p2,QG_p2,QD_O,in_O;
  ...
  AD iqs_p1 = type_p*(-QS_p1.fastAccessDx(0) * x[4] - QS_p1.fastAccessDx(1) * x[5] - QS_p1.fastAccessDx(3) *x[7]);

  // QS_p2 = f(Vsb_p2,Vgb_p2,Vdb_p2) = f(0,x[2]-x[0],x[3]-x[0]) Csd+Csg considered
  AD iqs_p2 = type_p*(-QS_p2.fastAccessDx(0) * x[4] - QS_p2.fastAccessDx(2) * x[6] - QS_p2.fastAccessDx(3) * x[7]);
  ...
  // Cgs + Cgd Considered
  AD iqg_p1 = type_p*(-QG_p1.fastAccessDx(0) * x[4] -  QG_p1.fastAccessDx(3) *x[7]);


  // QG_n1 = f(Vsb_n1,Vgb_n1,Vdb_n1) = f(0,x[1],x[3]
  //AD iqg_n1 = type_n*(QG_n1.fastAccessDx(1) * x[5] + QG_n1.fastAccessDx(3) * x[7]);
  // Cgs + Cgd Considered
  AD iqg_n1 = type_n*(-QG_n1.fastAccessDx(3) * x[7]);
  ...
  //Cgs + Cgd Considered
  AD iqg_p2 = type_p*(-QG_p2.fastAccessDx(0) * x[4] - QG_p2.fastAccessDx(3) *x[7]);

  // QG_n2 = f(Vsb_n2,Vgb_n2,Vdb_n2) = f(0,x[2],x[3]
  //AD iqg_n2 = type_n*(QG_n2.fastAccessDx(2) * x[6] + QG_n2.fastAccessDx(3) * x[7]);
  // Cgs + Cgd Considered
  AD iqg_n2 = type_n*(-QG_n2.fastAccessDx(3) * x[7]);
  ...
  // Cdg + Cds considered
  AD iqd_p1 = type_p*(-QD_p1.fastAccessDx(0) * x[4] - QD_p1.fastAccessDx(1) * x[5]);
  ...
  // Cdg + Cds considered
  AD iqd_p2 = type_p*(-QD_p2.fastAccessDx(0) * x[4] - QD_p2.fastAccessDx(2) * x[6]);

  AD iqd_O;
  if ((x[2] - x[1]) > 0)
  {
    //iqd_O = type_n*(QD_n1.fastAccessDx(1) * x[5] + QD_n1.fastAccessDx(3) * x[7]);
    iqd_O = type_n*(-QD_n1.fastAccessDx(1) * x[5]);  // Cdg +  Cds considered

  }
  else
  {
    //iqd_O =  type_n*(QD_n2.fastAccessDx(2) * x[6] + QD_n2.fastAccessDx(3) * x[7]);
    iqd_O =  type_n*(-QD_n2.fastAccessDx(2) * x[6]); // Cdg + Cds Considered
  }
  ...
  //cout<< "***********Currents***********"<< endl;
  //cout<< "iqs_p1:"<< iqs_p1 << endl;
  //cout<< "iqs_p2:"<< iqs_p2 << endl;
  //cout<< "iqg_n1:"<< iqg_n1 << endl;
  //cout<< "iqg_n2:"<< iqg_n2 << endl;
  //cout<< "iqg_p1:"<< iqg_p1 << endl;
  //cout<< "iqg_p2:"<< iqg_p2 << endl;
  //cout<< "iqd_O:"<< iqd_O << endl;
  //cout<< "iqd_p1:"<< iqd_p1 << endl;
  //cout<< "iqd_p2:"<< iqd_p2 << endl;
