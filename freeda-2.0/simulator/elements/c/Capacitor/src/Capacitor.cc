...
// Static members
const unsigned Capacitor::n_par = 3;

// Element information
ItemInfo Capacitor::einfo =
{
  "capacitor",
  "Linear capacitor",
...
Capacitor::Capacitor(const string& iname) : 
...
void Capacitor::init() throw(string&)
{
  if (time_d)
...
void Capacitor::fillMNAM(FreqMNAM* mnam)
{
...
void Capacitor::fillMNAM(TimeMNAM* mnam)
{
  // Ask my terminals the row numbers
  if (int_g) 
    mnam->setMAdmittance(getTerminal(0)->getRC(), getTerminal(1)->getRC(), int_g);
  mnam->setMpAdmittance(getTerminal(0)->getRC(), getTerminal(1)->getRC(), cap);
}
  
void Capacitor::svTran(TimeDomainSV* tdsv)
{
...
void Capacitor::deriv_svTran(TimeDomainSV* tdsv)
{
  tdsv->getJu()(0,0) = one;
