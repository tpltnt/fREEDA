...
const unsigned Cmos2Nor :: n_par = 71;

// Element Information
ItemInfo Cmos2Nor::einfo =
{
  "cmos2nor",
...
// Constructor
Cmos2Nor::Cmos2Nor(const string& iname): ADInterface(&einfo, pinfo, n_par, iname)
{
...
void Cmos2Nor::init() throw(string&)
{
  // Set Constants
  scale=1.0;
...
// Evaluate Function
void Cmos2Nor::eval(AD * x, AD * effort, AD * flow)
{

  //EKV Pmos and nmos models are used for building nor gate  which has four state variable :
  //x[0]- Vdd, x[1] - Vin1, x[2] - Vin2, x[3] - Vout
  //For Simplicity it is assumed : Bulk & Source of Nmos = 0, Bulk & Source of Pmos = Vdd
  //Pmos and nmos state varibles are found in terms of nor state variable:
  ...
  AD QS_n1, QG_n1,QS_p1,QG_p1,QS_n2,QG_n2,QS_p2,QG_p2,QD_O,in_O,QS_O;
  ...
  i_f_n1 = log(1.0 + exp((vp_n1 - type_n*0*vtT) / (2.0*vtT)))*log(1.0 + exp((vp_n1 - type_n*0) / (2.0*vtT))); //Vsb of nmos =0
  i_f_n2 = log(1.0 + exp((vp_n2 - type_n*0*vtT) / (2.0*vtT)))*log(1.0 + exp((vp_n2 - type_n*0) / (2.0*vtT))); //Vsb of nmos =0

  i_f_p1 = log(1 + exp((vp_p1 - type_p*0*vtT) / (2 * vtT)))*log(1 + exp((vp_p1 - type_p*0) / (2 * vtT))); // Vsb for pmos =0
  i_f_p2 = log(1 + exp((vp_p2 - type_p*0*vtT) / (2 * vtT)))*log(1 + exp((vp_p2 - type_p*0) / (2 * vtT))); // Vsb for pmos =0
  ...
  // condassign(QD_O,(x[2]-x[1]),QD_p2,QD_p1);
  //condassign(in_O,(x[2]-x[1]),id_p2,id_p1);
  //condassign(QS_O,(x[2]-x[1]),QS_p2,QS_p1);
  ...
  if(x[2] - x[1] > zero)
  {
    //QS_O = f(Vsb_p2,Vgb_p2,Vdb_p2) = f(0,x[2]-x[0],x[3]-x[0])
    iqs_O = type_p*(-QS_p2.fastAccessDx(3)* x[7] - QS_p2.fastAccessDx(2) * x[6] - QS_p2.fastAccessDx(0) * x[4] );  // Csd, Csg considered
  }
  else
  {
    //QS_O = f(Vsb_p1,Vgb_p1,Vdb_p1) = f(0,x[1]-x[0],x[3]-x[0]) // Csd, Csg considered
    iqs_O = type_p*(-QS_p1.fastAccessDx(3)* x[7] - QS_p1.fastAccessDx(1) * x[5] - QS_p1.fastAccessDx(0) * x[4]);
  }
  ...
  // QD_n1 = f(Vsb_n1,Vgb_n1,Vdb_n1) = f(0,x[1],x[3])
  // Cdg + Cds considered
  AD iqd_n1 = type_n*(-QD_n1.fastAccessDx(1) * x[5]);

  // QD_n2 = f(Vsb_n2,Vgb_n2,Vdb_n2) = f(0,x[2],x[3])
  // Cdg + Cds considered
  AD iqd_n2 = type_n*(-QD_n2.fastAccessDx(2) * x[6] );

  AD iqd_O;
  if ((x[2] - x[1]) > 0)
  {
    //QD_p2 = f(Vsb_p2,Vgb_p2,Vdb_p2) = f(0,x[2]-x[0],x[3]-x[0])
    iqd_O = type_p*(-QD_p2.fastAccessDx(2) * x[6] - QD_p2.fastAccessDx(0)*x[4]);  // Cdg +  Cds considered

  }
  else
  {
    // QD_p1 = f(Vsb_p1,Vgb_p1,Vdb_p1) = f(0,x[1]-x[0],x[3]-x[0])
    iqd_O =  type_p*(-QD_p1.fastAccessDx(1) * x[5]- QD_p1.fastAccessDx(0)*x[4]); // Cdg + Cds Considered

  }

  // Assign DC currents
  flow[0] = -(in_O + iqs_O); // AC+ DC current entering in VDD
  flow[1] = 0  + iqg_n1 + iqg_p1;   // AC + DC gate current Vin1
  flow[2] = 0  + iqg_n2 + iqg_p2;   // AC + DC gate current Vin2
  flow[3] = in_O + id_n1 + id_n2 + iqd_n1 + iqd_n2  + iqd_O ; // DC output current
