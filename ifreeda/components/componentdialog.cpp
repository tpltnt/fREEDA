			...
			: QDialog(parent, 0, TRUE, Qt::WDestructiveClose)
{
  ...
  Validator = new QRegExpValidator(this); //Replace the 2 lines above accept any string
  ...
  if((Comp->Model[0] == '.') &&
     ...
     (Comp->Model != ".out") && (Comp->Model != ".model") &&
     (Comp->Model != ".top") && (Comp->Model != ".bottom")) {
...
/*    if(checkSim)
    ...
    gp2 = new Q3GridLayout(Tab2, 9,2,5,5);
    myParent = Tab2;
  }
  else {   // no simulation component
    gp2 = new Q3GridLayout(0, 9,2,5,5);
    all->addLayout(gp2);
  }
  ...
  Q3VBoxLayout *v = new Q3VBoxLayout(); // stretchable placeholder
  v->addStretch(2);
  gp2->addLayout(v,7,1);

  Q3HBox *h4 = new Q3HBox(myParent);
  h4->setSpacing(5);
  ...
  for(Property *p = Comp->Props.last(); p != 0; p = Comp->Props.prev()) {
    ...
    correctDesc (str);
    new Q3ListViewItem(prop, p->Name, p->Value, s, str);
  }
